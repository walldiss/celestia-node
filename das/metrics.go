...
type metrics struct {
	...
	busyWorkers        asyncint64.UpDownCounter
...
func (sc *samplingCoordinator) initMetrics() error {
	sampled, err := meter.SyncInt64().Counter("das_sampled_headers_count",
		instrument.WithDescription("total amount of sampled headers"))
	if err != nil {
		return err
	}

	sampleTime, err := meter.SyncFloat64().Histogram("das_sample_time",
		instrument.WithDescription("header sampling time"))
	if err != nil {
		return err
	}

	failed, err := meter.SyncInt64().Counter("das_failed_headers_count",
		instrument.WithDescription("total amount of headers failed to be sampled"))
	if err != nil {
		return err
	}
	...
	busyWorkers, err := meter.AsyncInt64().UpDownCounter("das_busy_workers_count",
	...
	err = meter.RegisterCallback(
		...
		func(ctx context.Context) {
			...
			headOfSampledChain.Observe(ctx, int64(stats.HeadOfSampledChain))
...
func (m *metrics) observeSample(ctx context.Context, h *header.ExtendedHeader, sampleTime time.Duration, err error) {
	...
	m.sampled.Add(ctx, 1, attribute.Int("header_width", len(h.DAH.RowsRoots)))
}
